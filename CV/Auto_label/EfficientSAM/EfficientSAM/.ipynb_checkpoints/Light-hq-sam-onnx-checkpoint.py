...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(HQSAM_CHECKPOINT_PATH, map_location=torch.device('cpu'))
    ...
    onnx_model_path = "./light_hq_sam_onnx.onnx"
    ...
    for i,(img,seg) in enumerate(zip(img_file,seg_file)):
        ...
        interm_embeddings = torch.stack(sam_predictor.interm_features).cpu().numpy()
        ...
        for j,box in enumerate(bbox_arrays):
