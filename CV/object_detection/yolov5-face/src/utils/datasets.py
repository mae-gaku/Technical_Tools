...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    im = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # add border
    # im = cv2.copyMakeBorder(im, 0, new_shape[0] - im.shape[0], 0, new_shape[1] - im.shape[1], cv2.BORDER_CONSTANT, color)
    top_bottom, left_right = int((new_shape[0] - im.shape[0]) / 2), int((new_shape[1] - im.shape[1]) / 2)
    im = cv2.copyMakeBorder(im, top_bottom, top_bottom, left_right, left_right, cv2.BORDER_CONSTANT, color)
