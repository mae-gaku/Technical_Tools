...
def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, max_det=300, classes=None):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        j = np.argmax(x[:, 15:], axis=1).reshape(-1, 1).astype(np.float32)
        conf = np.max(x[:, 15:], axis=1, keepdims=True)
        x = np.concatenate((box, conf.reshape(-1, 1), j), axis=1)

        # Filter by class
        if classes is not None:
            # x = x[(x[:, 5:6] == torch.tensor(classes, device=x.device)).any(1)]
            condition = np.any(x[:, 5:6] == classes, axis=1)
            x = x[condition]
        ...
        def nms(boxes, scores, iou_thres):
            keep = np.zeros(scores.shape, dtype=np.bool)
