...
if __name__ == "__main__":
    ...
    output_image_dir = '/media/sf_virtualbox/My_code/work_nemoto_codes/trash_0213_v1/images1'

    if not os.path.exists('augmentation/mosaic_labels'):  
        os.makedirs('augmentation/mosaic_labels')    

    # output dir for new boxes
    output_label_dir = '/media/sf_virtualbox/My_code/work_nemoto_codes/trash_0213_v1/labels1'

    """
        Note: Image and label have the same name. For example: image_1.jpeg - image_1.txt
    """
    print('Processing...')
    image_dir = '/media/sf_virtualbox/My_code/work_nemoto_codes/trash_0213_v1/images'
    label_dir = '/media/sf_virtualbox/My_code/work_nemoto_codes/trash_0213_v1/labels'

    
    # get a list of all images
    import glob
    import cv2
    img = glob.glob(image_dir + "/*")
    ...
    # for i in range(image_num):
        ...
        # list_image_names = os.listdir(image_dir)
        image_file_list = random.sample(list_image_names, k=4)
        print("image_file_list",image_file_list)
        print("list_image_names",len(list_image_names))
    ...
    for i in list_image_names:
        # print("i",i)
        shutil.copy(image_dir+"/" + i, output_image_dir)
        basename_without_ext = os.path.splitext(os.path.basename(i))[0]
        shutil.copy(label_dir+"/" + basename_without_ext + ".txt", output_label_dir)