...
from Tag2Text.models.bert import BertConfig, BertModel, BertLMHeadModel
from Tag2Text.models.vit import VisionTransformer
from Tag2Text.models.swin_transformer import SwinTransformer
from Tag2Text.data.ram_tag_list_threshold import ram_class_threshold

from Tag2Text.models.utils import *
...
class RAM(nn.Module):
    ...
    def generate_tag(self,
        ...
        # tholod
        # targetsの値が1.0となるインデックスを取得
        thresholded_indices = torch.nonzero(targets == 1.0).squeeze()

        thresholded_logits_list = [logits[0, idx].tolist() for idx in thresholded_indices]
        print("thresholded_logits_list", thresholded_logits_list)
        thresholded_list = [torch.tensor(logit) for logit in thresholded_logits_list]
        print("thresholded_list", thresholded_list)
        
        sigmoid_thresholded_logits_list = [torch.sigmoid(torch.tensor(logit)) for logit in thresholded_logits_list]
        # print("thresholded_logits_list", thresholded_logits_list)
        ...
        return tag_output, tag_output_chinese, sigmoid_thresholded_logits_list
