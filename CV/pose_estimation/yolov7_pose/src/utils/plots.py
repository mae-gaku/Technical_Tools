    ...
    for i, o in enumerate(output):
        if not output[0].tolist():
            ...
            for index, (*box, conf, cls) in enumerate(o.detach().cpu().numpy()):
                # box=x, y, w, h / conf = conf / cls =  class_id
                targets.append([i, cls, *list(*xyxy2xywh(np.array(box)[None])), conf, *list(kpts.detach().cpu().numpy()[index])])
                xywh = list(*xyxy2xywh(np.array(box)[None]))
   
    return np.array(targets),xywh
...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    return x_coord_list, y_coord_list, coord_list
